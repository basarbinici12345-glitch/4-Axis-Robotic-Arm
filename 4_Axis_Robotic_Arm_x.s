...
#define TIM2_CCR2     (*(volatile uint32_t*)(TIM2_BASE + 0x38))  // YENİ: Gripper
...
#define UPPER_LENGTH   20.0f
#define LOWER_LENGTH   20.0f
#define PWM_HOME       1500
#define PWM_MIN         500
#define PWM_MAX        2500

// --- Gripper mekanik sınırları ---
#define GRIPPER_KAPALI  950   // tam kapalı
#define GRIPPER_ACIK    1900   // tam açık
...
volatile float    interp_step_gripper   = 0.0f;   // YENİ
...
volatile uint32_t interp_h_gripper      = GRIPPER_ACIK;  // YENİ
...
// ============================================================
// GRİPPER PWM CLAMP — mekanik sınırları zorla
// ============================================================
static uint32_t gripper_clamp(uint32_t pwm) {
    if (pwm < GRIPPER_KAPALI) pwm = GRIPPER_KAPALI;
    if (pwm > GRIPPER_ACIK)   pwm = GRIPPER_ACIK;
    return pwm;
}

// ============================================================
// İNTERPOLASYON BAŞLAT — gripper eklendi
// ============================================================
void interpolation_baslat(uint32_t h_hip,  uint32_t h_omuz,
                           uint32_t h_dirsek, uint32_t h_gripper,
                           uint32_t adim) {
    interp_h_hip     = h_hip;
    interp_h_omuz    = h_omuz;
    interp_h_dirsek  = h_dirsek;
    interp_h_gripper = h_gripper;
    interp_toplam    = adim;
    interp_adim      = 0;
    ...
    interp_step_gripper = (float)((int32_t)h_gripper - (int32_t)mevcut_gripper_pwm) / (float)adim;

    interp_aktif = 1;
}

// ============================================================
// TIM2 UPDATE INTERRUPT — gripper kanalı eklendi
// ============================================================
void TIM2_IRQHandler(void) {
    ...
    if (interp_adim < interp_toplam) {
        ...
        mevcut_gripper_pwm = (uint32_t)((float)mevcut_gripper_pwm + interp_step_gripper);

        TIM2_CCR1 = mevcut_hip_pwm;
        TIM3_CCR1 = mevcut_gripper_pwm;
        ...
        mevcut_gripper_pwm = interp_h_gripper;

        TIM2_CCR1 = interp_h_hip;
        TIM3_CCR1 = interp_h_gripper;
...
// ============================================================
// G-CODE İŞLEYİCİ — gripper komutları eklendi
// ============================================================
void gcode_isle(char* buf) {

    // G28 — Home (gripper de kapanır)
    if (buf[0]=='G' && buf[1]=='2' && buf[2]=='8') {
        hedef_hip_pwm     = PWM_HOME;
        hedef_omuz_pwm    = PWM_HOME;
        hedef_dirsek_pwm  = PWM_HOME;
        hedef_gripper_pwm = GRIPPER_ACIK;
        mesgul = 1;
        interpolation_baslat(PWM_HOME, PWM_HOME, PWM_HOME, GRIPPER_ACIK, 50);
        uart_send_str("OK HOME\r\n");
        return;
    }

    // M114 — Pozisyon raporu (gripper dahil)
    if (buf[0]=='M' && buf[1]=='1' && buf[2]=='1' && buf[3]=='4') {
        ...
        uart_send_str(" G:"); uart_send_int(mevcut_gripper_pwm);  // YENİ
        uart_send_str("\r\n");
        return;
    }

    // M10 — Gripper aç  (tam açık = GRIPPER_ACIK)
    if (buf[0]=='M' && buf[1]=='1' && buf[2]=='0') {
        hedef_gripper_pwm = GRIPPER_ACIK;
        mesgul = 1;
        interpolation_baslat(mevcut_hip_pwm, mevcut_omuz_pwm,
                             mevcut_dirsek_pwm, GRIPPER_ACIK, 20);
        return;
    }

    // M11 — Gripper kapat  (tam kapalı = GRIPPER_KAPALI)
    if (buf[0]=='M' && buf[1]=='1' && buf[2]=='1') {
        hedef_gripper_pwm = GRIPPER_KAPALI;
        mesgul = 1;
        interpolation_baslat(mevcut_hip_pwm, mevcut_omuz_pwm,
                             mevcut_dirsek_pwm, GRIPPER_KAPALI, 20);
        return;
    }

    // G0 / G1 — Hareket + isteğe bağlı gripper (E parametresi)
    if (buf[0]=='G' && (buf[1]=='0' || buf[1]=='1')) {
        float R        = parse_param(buf, 'X');
        float z        = parse_param(buf, 'Y');
        float hip_deg  = parse_param(buf, 'Z');
        float grip_pwm = parse_param(buf, 'E');   // YENİ: E=PWM değeri
        ...
        // E parametresi geldiyse gripper hedefini güncelle
        if (grip_pwm > -9000.0f)
            hedef_gripper_pwm = gripper_clamp((uint32_t)grip_pwm);

        mesgul = 1;
        interpolation_baslat(hedef_hip_pwm, hedef_omuz_pwm,
                             hedef_dirsek_pwm, hedef_gripper_pwm, 20);
        return;
    }
...
// ============================================================
// MAIN — PA1 pini ve TIM2_CH2 eklendi
// ============================================================
int main(void) {
    SCB_CPACR |= (0xF << 20);

    RCC_AHB1ENR |= (1 << 0) | (1 << 1);
    RCC_APB1ENR |= (1 << 0) | (1 << 1) | (1 << 18);


    // PA0(Hip), PA2(Omuz), PA3(Dirsek) ve PA6(Gripper) -> AF Mode
    GPIOA_MODER |= (2 << 0) | (2 << 4) | (2 << 6) | (2 << 12);

    // PA0, PA2, PA3 -> AF1 (1)  |  PA6 -> AF2 (2)
    GPIOA_AFRL  |= (1 << 0) | (1 << 8) | (1 << 12) | (2 << 24);
    ...
    // TIM2: SADECE CH1, CH3 ve CH4 aktif
    TIM2_PSC    = 16 - 1;
    TIM2_ARR    = 20000 - 1;
    TIM2_CCMR1 |= (6 << 4)  | (1 << 3);              // CH1 PWM mode1
    TIM2_CCMR2 |= (6 << 4)  | (1 << 3) | (6 << 12) | (1 << 11); // CH3 ve CH4 PWM mode1
    TIM2_CCER  |= (1 << 0) | (1 << 8) | (1 << 12);   // CH1, CH3, CH4 Enable
    TIM2_DIER  |= (1 << 0);
    NVIC_ISER0 |= (1 << 28);
    TIM2_CR1   |= (1 << 0);

    // TIM3: SADECE CH1 aktif (PA6 - Gripper)
    TIM3_PSC    = 16 - 1;
    TIM3_ARR    = 20000 - 1;
    TIM3_CCMR1 |= (6 << 4) | (1 << 3);               // CH1 PWM mode 1
    TIM3_CCER  |= (1 << 0);                          // CH1 Enable
    TIM3_CR1   |= (1 << 0);

    TIM2_CCR1 = PWM_HOME;
    TIM3_CCR1 = GRIPPER_KAPALI;   // gripper kapalı başlasın
    TIM2_CCR3 = PWM_HOME;
    TIM2_CCR4 = PWM_HOME;
